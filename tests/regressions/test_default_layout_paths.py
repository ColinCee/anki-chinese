...
def test_default_layout_paths_match_the_documented_repo_structure() -> None:
    ...
    assert project_root / 'data' / 'source' / 'All Decks.txt' == config.SOURCE_DECK_PATH
    assert project_root / 'data' / 'manual' / 'overrides.json' == config.OVERRIDES_PATH
    assert project_root / 'data' / 'manual' / 'example_words.json' == config.EXAMPLE_WORDS_PATH
    assert project_root / 'data' / 'reference' / 'cedict_1_0_ts_utf-8_mdbg.txt' == config.CEDICT_PATH
