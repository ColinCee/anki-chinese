...
from anki_chinese.audio.provider import AudioGenerationProfile, ProviderCapabilities
...
class StubTTSProvider:
    def __init__(
        ...
        *,
        ...
        self.generated_audio_dir: Path | None = None
    ...
    def generate_mandarin(self, hanzi: str, pinyin: str, *, force: bool = False) -> str:
        ...
        filename = f"cmn_{hanzi}_{pinyin.replace(' ', '_')}.mp3"
        tag = f"[sound:{filename}]"
        self._write_audio_file(filename)
    ...
    def generate_plain_mandarin(self, text: str, *, force: bool = False) -> str:
        ...
        filename = f"preview_cmn_{text.replace(' ', '_')}.mp3"
        tag = f"[sound:{filename}]"
        self._write_audio_file(filename)
        self.valid_audio_tags.add(tag)
    ...
    def generate_cantonese(
        ...
        *,
        ...
        filename = f"yue_{hanzi}_{jyutping.replace(' ', '_')}.mp3"
        tag = f"[sound:{filename}]"
        self._write_audio_file(filename)
        self.valid_audio_tags.add(tag)
    ...
    def generate_sentence_audio(
        ...
        *,
        ...
        filename = f"cmn_sentence_{sentence}.mp3"
        tag = f"[sound:{filename}]"
        self._write_audio_file(filename)
        self.valid_audio_tags.add(tag)
    ...
    def _write_audio_file(self, filename: str) -> None:
        if self.generated_audio_dir is None:
            return
        self.generated_audio_dir.mkdir(parents=True, exist_ok=True)
        (self.generated_audio_dir / filename).write_bytes(b"ID3")
    ...
    def make_runtime(
        *,
        ...
        if isinstance(active_tts_provider, StubTTSProvider):
            active_tts_provider.generated_audio_dir = generated_audio_dir
        ...
        return AppRuntime(
            ...
            audio_manifest_path=tmp_path / "data" / "state" / "audio_manifest.json",
